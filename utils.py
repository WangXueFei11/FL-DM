...
def get_dataset(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        dst_train = datasets.ImageFolder(data_path+'/train', transform=transform) # no augmentation
        ...
        dst_test = datasets.ImageFolder(data_path+'/val', transform=transform)
...
# 根据输入的模型名称创建对应的神经网络架构
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True, depth=3, width=128, norm="instancenorm", args=None):
...
def epoch_global(mode, dataloader, net, optimizer, criterion, args, aug, it=0):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    # criterion = criterion.to(args.device)
...
def get_eval_pool(eval_mode, model, model_eval):
    ...
    elif "CIFAR" in eval_mode:
        model_eval_pool = [model, "AlexNetCIFAR", "ResNet18CIFAR", "VGG11CIFAR", "ViTCIFAR"]
    elif "imagenet" in eval_mode:
        model_eval_pool = [model, "ResNet18ImageNet", "ResNet34"]
    else:
...
def get_logger(filename, distributed_rank, verbosity=1, name=None):
