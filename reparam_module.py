...
class ReparamModule(nn.Module):
    def _get_module_from_name(self, mn):
        ...
        for p in mn.split('.'):
            m = getattr(m, p)
        return m
    # 主要功能是将输入的模块参数进行扁平化处理，并重新注册为一个单一的参数
    def __init__(self, module):
        ...
        param_numels = []
        param_shapes = []
        # 遍历模块中的所有子模块及其参数，记录每个参数的信息
        for mn, m in self.named_modules():
            for n, p in m.named_parameters(recurse=False):
                if p is not None:
                    # 如果参数是共享的，则记录其来源；否则，记录参数信息并将其加入待处理列表。
        ...
        # flatten
        # 扁平化参数：将所有参数展平为一维张量，并拼接成一个大的一维张量。
        ...
        # 调用 _unflatten_param 方法将扁平化的参数重新映射回原始结构
